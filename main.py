...
# 積立期間(何ヶ月間）
funding_period = st.sidebar.number_input('月数', 1, 360, 12,1)

tax = st.sidebar.number_input('購入時手数料(%)',0.0,None,3.3,0.1)/100
...
amounts = {
    '投資のソムリエ': 10000,
    'ピクテ・マルチアセット・アロケーション・ファンド': 10000, 
    'ひふみプラス': 10000,
    '三井住友・中小型株ファンド': 10000,
    'ＪＰＭアジア株・アクティブ・オープン': 10000,
    'キャピタル世界株式ファンド': 'JP90C00052C3',
    'ｎｅｔＷＩＮ　ＧＳテクノロジー株式ファンドＢコース（為替ヘッジなし）': 10000,
    'ベイリー・ギフォード世界長期成長株ファンド': 10000
}
...
for key in amounts.keys():
    amount_month = amount_month + amounts[key]

st.sidebar.write(f"""
## 毎月 **{amount_month}円** 積立
""")
        ...
        for itrust in isin_codes.keys():
            ...
            for i in range(len(df_reserve)):
                if df_reserve['ファンド名'].values[i] == itrust:
                    df_reserve['購入口数'].values[i] = round(amounts[itrust]*(1-tax) / df_reserve['基準価額(円)'].values[i] *10000)
                    ...
                    df_reserve['投資額'].values[i] = amounts[itrust]
                    df_reserve['投資総額'].values[i] = old2 + df_reserve['投資額'].values[i]

                    df_reserve['損益'].values[i] = df_reserve['評価額'].values[i] - df_reserve['投資総額'].values[i]
                    df_reserve['損益率'].values[i] = round(df_reserve['損益'].values[i] / df_reserve['投資総額'].values[i] * 100 ,2)
